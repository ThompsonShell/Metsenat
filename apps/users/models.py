...
class CustomUser(AbstractUser):
    ...
    params:
        balance:
            sponsor: sum of appeals amount (status=CONFIRMED)
            student: contract amount
        available:
            sponsor: sum of appeals available (status=CONFIRMED)
            student: collected amount (sum of related sponsors amount)
    """
    ...
    balance = models.DecimalField(max_digits=50,
                                  decimal_places=5,
                                  default=1,
                                  validators=[MinValueValidator(1)])

    available = models.DecimalField(max_digits=50,
                                    decimal_places=5,
                                    default=1,
                                    validators=[MinValueValidator(1)])
    ...
    phone_number = models.CharField(max_length=13, help_text="please enter only 13 number",
                                    validators=[validate_phone_number], unique=True)
    ...
    def set_balance_available(self):
        from apps.appeals.models import Appeal

        if self.role == self.Role.SPONSOR:
            self.balance = Appeal.objects.filter(status=Appeal.Status.CONFIRMED
                                                 ).aggregate(models.Sum('amount'))['amount__sum'] or 0
            self.available = Appeal.objects.filter(status=Appeal.Status.CONFIRMED
                                                   ).aggregate(models.Sum('available'))['available__sum'] or 0
        elif self.role == self.Role.STUDENT:
            self.balance = self.university.contract_amount
            self.available = self.sponsors.aggregate(models.Sum('amount'))['amount__sum'] or 0

    def save(self, *args, **kwargs):
        if self.role == self.Role.STUDENT and self.balance is None:
            self.balance = self.university.contract_amount

        self.set_balance_available()
        super().save(*args, **kwargs)

    def clean(self):
        if self.role == self.Role.STUDENT and self.degree not in [self.StudentDegree.BACHELOR,
                                                                  self.StudentDegree.MASTERS]:
